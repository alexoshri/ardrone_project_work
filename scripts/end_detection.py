        ...
        if (chosen_cnt is None):  # path is not visible
            ...
            try:
                ...
                angle = -np.angle((center2_x - center1_x) + (center2_y - center1_y) * 1j,
                                  deg=True) - 90  # angle calculated between arrow and the vertical axis (POSITIVE = arrow to the LEFT)
                # angle_perp = angle - 90
                ...
                img_calc.turn_indicator_distance = ((x_nearest_pt_on_line - nearest_pt[1]) ** 2 + (
                y_nearest_pt_on_line - nearest_pt[0]) ** 2) ** 0.5
                ...
                cv2.putText(res1, 'distance: {}'.format(img_calc.distance), (w / 2, 100), cv2.FONT_ITALIC, 1,
                            (255, 255, 255), 2)
                cv2.putText(res1, 'angle: {0:.3f}'.format(img_calc.angle), (w / 2, 50), cv2.FONT_ITALIC, 1,
                            (255, 255, 255), 2)
                cv2.putText(res1, 'turn distance: {}'.format(img_calc.turn_indicator_distance), (w / 2, 200),
